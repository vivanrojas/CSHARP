...
namespace EndesaBusiness.contratacion.Redshift
{
    public class Inventario
    {
        ...
        public Inventario(List<string> lista_cups_20)
        {
            param = new EndesaBusiness.utilidades.Param("t_ed_h_ps_param", MySQLDB.Esquemas.CON);
            ss_pp = new utilidades.Seguimiento_Procesos();
            ficheroLog = new logs.Log(Environment.CurrentDirectory, "logs", "CON_Copia_PS_AT_ROSETTA");
        ...
        private void Carga(List<string> lista_cups_20)
        {
            MySQLDB db;
            ...
            command = new MySqlCommand(ConsultaMySQL(lista_cups_20), db.con);
        ...
        public void CopiaDatos()
        {
            ...
            try
            {
                ...
                if (param.GetValue("estado_proceso") != "en ejecución" && (ultima_actualizacion.Date < DateTime.Now.Date || param.GetValue("estado_proceso") == "lanzar"))
                {
                    ...
                    while (r.Read())
                    {
                        ...
                        if (firstOnly)
                        {
                            sb.Append("REPLACE INTO t_ed_h_ps_tmp");
                    ...
                    if (!hay_error)
                    {
                        strSql = "replace into t_ed_h_ps_hist"
                        + " select t.* from t_ed_h_ps t";
                        ficheroLog.Add(strSql);
                        dbmy = new servidores.MySQLDB(servidores.MySQLDB.Esquemas.CON);
                        commandmy = new MySqlCommand(strSql, dbmy.con);
                        commandmy.ExecuteNonQuery();
                        ...
                        strSql = "replace into t_ed_h_ps"
                        + " select * from t_ed_h_ps_tmp t";
                        ficheroLog.Add(strSql);
                        dbmy = new servidores.MySQLDB(servidores.MySQLDB.Esquemas.CON);
                        commandmy = new MySqlCommand(strSql, dbmy.con);
                        commandmy.ExecuteNonQuery();
                    ...
                    ss_pp.Update_Fecha_Fin("Contratación", "Copia Inventario BI PSAT_ROSETTA",
                        "Copia Inventario BI PSAT_ROSETTA");
            ...
            }catch(Exception ex)
            {
                ...
                ss_pp.Update_Comentario("Contratación", "Copia Inventario BI PSAT_ROSETTA", "Copia Inventario BI PSAT_ROSETTA", ex.Message);
            }
        }
        ...
        private DateTime UltimaActualizacion()
        {
            ...
            OdbcCommand command;
            OdbcDataReader r;
            string strSql = "select max(fh_act_dmco) as max_fecha from ed_owner.t_ed_h_ps";
        ...
        private string ConsultaMySQL(List<string> lista_cups_20)
        {
            StringBuilder sb = new StringBuilder();
            ...
            if(lista_cups_20.Count > 0)
            {
                ...
                sb.Append(" FROM t_ed_h_ps where");
