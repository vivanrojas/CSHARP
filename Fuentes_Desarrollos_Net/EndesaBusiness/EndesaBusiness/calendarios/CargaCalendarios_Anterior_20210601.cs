...
{
    public class CargaCalendarios_Anterior_20210601
    {
        ...
        public CargaCalendarios_Anterior_20210601()
        {
        ...
        public CargaCalendarios_Anterior_20210601(DateTime fromDate, DateTime toDate)
        {
        ...
        public CargaCalendarios_Anterior_20210601(DateTime fromDate, DateTime toDate, string tarifa, string territorio)
        {
            CargaListaTarifas(fromDate, toDate);
            CargaListaFestivos(fromDate, toDate);
            CargaDefinicionCalendario(fromDate, toDate);
        ...
        public int GetPeriodoTarifa(string tarifa, string territorio, DateTime f, int ph)
        {
            ...
            {
                if (tarifa == "2.0" || tarifa == "2.1" || tarifa == "2.0A" || tarifa == "2.1A")
                {
                    p = 1;
                }
                else if (tarifa == "2.0DHA" || tarifa == "2.1DHA")
                {
                    territorio = "TODOS";
                    buscarPeriodoTarifario = true;
                    laborable = false;
                }
                else if (tarifa.Contains("3.0A"))
                {
                    laborable = false;
                    buscarPeriodoTarifario = true;
                }
                else if (tarifa.Contains("3.1"))
                {
                    if (!EsLaborable(f) || EsFestivo(f))
                    {
                        laborable = false;
                    }
                    else
            ...
            {
                if (tarifa.ToUpper() == "6.X" && (!EsLaborable(f) || EsFestivo(f)))
                {
                    p = 6;
                }
                else
        ...
        {
            DateTime fecha;
            fecha = lf.Find(z => z.Equals(f));
            return fecha.Equals(f);
        }
