...
namespace EndesaBusiness.utilidades
{
    public class Fechas
    {
        logs.Log ficheroLog = new logs.Log(Environment.CurrentDirectory, "logs", "GBL_Fecha");
        ...
        public string MesTexto(DateTime f)
        {
            string mes = "";
            switch (f.Month)
            {
        ...
        private void CargaFestivos()
        {
            ...
            {
                ficheroLog.AddError("CargaFestivos: " + e.Message);
            }
        }
        ...
        public bool EsLaborable()
        {
            DateTime hoy = new DateTime();
            hoy = DateTime.Now;
            return ((int)hoy.DayOfWeek != 0 && (int)hoy.DayOfWeek != 6);
        }
        ...
        public DateTime FD_MesTrabajo()
        {
            DateTime mesAnterior = this.UltimoDiaHabil().AddMonths(-1);            
