    ...
    public class Sofisticados
    {
        ...
        private Dictionary<string, EndesaEntity.contratacion.ContratosPS> Carga_Actual()
        {
            ...
            try
            {
                ...
                strSql = "SELECT ps.IDU AS CCOUNIPS, ps.CUPS20,"
                    + " NULL AS FINICPLA, NULL AS CNUMSCPS, ps.estadoCont AS TESTCONT,"
                    + " NULL AS FFINVESU, DATE_FORMAT(ps.fAltaCont, '%Y-%m-%d') AS FALTACON, DATE_FORMAT(ps.fBajaCont, '%Y-%m-%d') AS FBAJACON,"
                    + " DATE_FORMAT(ps.FPSERCON, '%Y-%m-%d') AS FPSERCON, NULL AS FPSERCON, NULL AS CCOMPOBJ"
                    + " FROM cont.PS_AT ps INNER JOIN"
                    + " fact.t_ed_h_sap_tam_agora t ON"
                    + " t.cd_cups = ps.CUPS20"
                    + " WHERE ps.estadoCont IN (1,3,4,8)"
                    + " AND t.agora = 'S'"
                    + " GROUP BY ps.CUPS20";
